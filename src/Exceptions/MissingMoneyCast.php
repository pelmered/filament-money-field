...
class MissingMoneyCast extends RuntimeException
{
    public function __construct(string $model, string $attribute)
    {
        parent::__construct(strtr(
            ...
            .'Add \'{attribute}\' => \{moneyCast}::class and \'{attribute}{suffix}\' => \{currencyCast}::class '
            .'to the $casts of the model. See https://github.com/pelmered/filament-money-field#casts',
            [
                '{attribute}'    => $attribute,
                '{model}'        => $model,
                '{suffix}'       => (string) config('larapara.currency_column_suffix', '_currency'),
                '{moneyCast}'    => MoneyCast::class,
                '{currencyCast}' => CurrencyCast::class,
            ]
        ));
    }
}
